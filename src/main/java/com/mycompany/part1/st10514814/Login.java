...
// Validates username format: must contain underscore and be max 5 characters
    public boolean checkUserName(String username) {
        ...
        if (username.contains("_") && username.length() <= 5) {
            ...
            result = "Username is not correctly formatted; please ensure your username contains "
...
// Validates password strength using regex pattern matching
    public boolean checkPaswordComplexity(String password) {
        //Password regex requires: minimum 8 characters, at least 1 uppercase, 1 lowercase, 1 number, and 1 special character 
        //Password validation using regex: (Stack Overflow, 2013)
        ...
        if (password.matches(PASSWORD_REGEX)) {
            ...
            result = "Password not correctly formatted; please ensure that the password contains at "
                    + "least eight characters, a capital letter, a number, and a speacial character";
            checkPassword = false;
        }
...
// Validates South African cell phone number format
    public boolean checkCellPhoneNumber(String cellNumber) {
        // South African cell phone format -
        //South African cellphone number validation:(Stack Overflow, 2010)
        ...
        if (cellNumber.matches(CELL_REGEX)) {
            ...
            result = "Cell phone number incorrectly formatted "
                    + "or does not contain internatioal code";
            checkNumber = false;
        }
